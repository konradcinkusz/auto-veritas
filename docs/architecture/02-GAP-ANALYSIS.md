...
| 2 | Calls `AddServiceDefaults()` and `MapDefaultEndpoints()` | Met | `src/AutoVeritas.OffersService/Program.cs` |
| 3 | Exposes `/health` and `/alive`; platform check points at health | Met — every platform check targets readiness (`/health/ready`) | `src/AutoVeritas.ServiceDefaults/Extensions.cs`, `flyio/*.fly.toml` |
| 4 | Emits OTLP traces, metrics and logs | Met in code; no collector endpoint configured in the Fly topology yet | `src/AutoVeritas.ServiceDefaults/Extensions.cs`; open row in [DEVIATIONS.md](DEVIATIONS.md) |
...
| 10 | Every optional integration has a working no-op or fallback | Met by scope: the system has no optional integrations yet; the OTLP exporter registers only when its endpoint is set | `src/AutoVeritas.ServiceDefaults/Extensions.cs` |
...
| 13 | Outbound `HttpClient`s carry the standard resilience handler with explicit timeouts | Met with a caveat: every client gets `AddStandardResilienceHandler()` at the library's default timeouts — the service makes no outbound service-to-service calls today, so no per-criticality overrides exist yet. The BFF proxy carries its own 120 s abort → 504 | `src/AutoVeritas.ServiceDefaults/Extensions.cs`, `packages/web-kit/src/proxy.ts` |
| 14 | `Program.cs` is a manifest; wiring in `ServiceCollectionExtensions` | Met — every block is one extension call | `src/AutoVeritas.OffersService/Program.cs` |
