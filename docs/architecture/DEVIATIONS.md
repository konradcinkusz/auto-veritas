...
| 2026-08-24 | Secret scanning runs in CI only; the baseline requires gitleaks as a pre-commit hook AND a CI job. | P5 / REPO-BASELINE | Add a committed pre-commit config (e.g. `pre-commit` with the gitleaks hook) and document the one-time `pre-commit install`. |
| 2026-08-24 | No manual scale workflow exists (`flyio-scale.yml`); only deploy and destroy are covered. | FLY-IO-DEPLOYMENT | Add a `workflow_dispatch` scale workflow that excludes the stateful postgres app, when the first real scaling need appears. |
| 2026-08-24 | The CodeQL checks fail at the SARIF-upload step: code scanning is not enabled in this repository's settings ("Code scanning is not enabled for this repository"). The analysis itself runs clean; the workflow and its permissions are correct. | REPO-BASELINE (SAST in CI) | Owner action: Settings → Security → Code security and analysis → enable code scanning (a private repo may need GitHub Advanced Security or going public). Delete this row once the checks upload. |
