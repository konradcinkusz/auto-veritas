...
| 2026-08-24 | No PR preview environments — the E2E tiers run against a compose-per-run stack instead. (The smoke and core-regression tiers now both exist; the nightly extended tier does not.) | P13 | Preview environments per PR-PREVIEW-ENVIRONMENTS when review traffic justifies them; the extended tier when the suite outgrows one browser. |
